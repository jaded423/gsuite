...
- Reuse the pre-granted token file at ~/.config/gspace/tokens.json.
...
- The browser consent flow is only invoked by `gspace auth` — the server
...
log = logging.getLogger("gspace.auth")
...
def get_credentials(force_refresh: bool = False) -> Credentials:
    ...
    if tokens is None:
        raise AuthError(
            f"No tokens at {TOKENS_PATH}. Run `gspace auth` to authorize."
        )
    ...
    if force_refresh or _needs_refresh(tokens):
        if not creds.refresh_token:
            raise AuthError(
                "Token file has no refresh_token. Run `gspace auth`."
            )
...
def run_auth_flow(scopes: list[str]) -> Credentials:
    ...
    Called by `gspace auth`. Uses the installed-app / local-server flow,
...
def scope_report() -> dict[str, Any]:
    """Return a snapshot of scope state for `gspace status`."""
    s = load_settings()
