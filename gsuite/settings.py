...
The user controls what the MCP can do via ~/.config/gsuite/settings.json.
...
aren't in the current tokens triggers a "run `gsuite auth`" message and
...
CONFIG_DIR = Path(
    os.environ.get("GSUITE_CONFIG_DIR", Path.home() / ".config" / "gsuite")
)
