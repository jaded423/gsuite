---
type: log
title: gsuite — design proposal & build log (archived)
tags: [gsuite, history, design, proposal, build-log]
related: [index, changelog]
---

# gsuite — design proposal & build log (archived)

> **Archived out of `CLAUDE.md` on 2026-07-07** during the Tier-2 slim-down, kept for the
> "why." This is the original design proposal + dated build log. It is **NOT the live
> state** — dated tool counts below (14, 37) are point-in-time snapshots; the current
> roster is `gsuite tools` / [index.md](index.md) (63 tools / 16 flags). Older `gspace`
> names and `~/projects/mcp/gspace/` paths are preserved as-written. The live feature-flag
> mechanics and multi-org OAuth model stayed in [../CLAUDE.md](../CLAUDE.md).
>
> **Still authoritative despite living here:** the *"Key learnings"* Gmail-filter gotchas
> (negatedQuery coordination, 1000-ID batch cap, label→ID resolution, verbatim criteria) —
> they describe how the live filter/rule engine must behave, not just history.
...
Additional feature flags exist (`drive.read/write`, `docs`, `sheets`, `slides`, `calendar`, `tasks`, `gmail.read`, `gmail.send`) — scopes pre-granted, tools landed in Phase 5 (Drive/Docs/Sheets/Slides/Calendar) and later (`gmail.read`/`gmail.send` read/compose/attachments). See docs/index.md for the live roster.
...
- ~~Rename `gspace` → `gsuite` (deferred until after burn-in).~~ **Done 2026-06-15.**
